...
iJEP 7 是典型的前后端分离微服务应用，开发数据库使用 MySQL，后端服务依赖于 Redis 和 Elasticsearch，服务注册与发现中心使用 Consul，前端采用 Vue + Element，部署于 Nginx 之上。
...
首先去Oracle官网下载[ Java SE Development Kit 8u201](https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html)，然后双击运行下载回来的**jdk-8u201-windows-x64.exe**文件。
...
![image-20211211224009559](images/image-20211211224009559.png)
...
在开发阶段，为了方便系统部署，我们并没有使用 Apollo 配置中心，而是在工程中使用 application-dev.yml 文件对数据库、redis、elasticsearch等外部资源使用主机名的方式进行了配置，所以你需要在开发机上根据实际机器 ip 地址信息对主机名进行配置。
...
将 ijep-router-gateway.jar、ijep-router-gateway.jar、ijep-service-bpm-7.0.0-SNAPSHOT.jar 和 ijep-service-customer-7.0.0-SNAPSHOT.jar 拷贝到 `C:\Users\Kevin\iJEP7\run-env\ijep-jars` 目录下，完成后端服务的部署；

将打包好的前端应用拷贝到 `C:\Users\Kevin\iJEP7\run-env\nginx-1.20.2\html` 目录下，完成前端控制台的部署。
...
为了方便演示，我们为 iJEP 7 配备了启动批处理文件 start_01.bat 和 start_02.bat，因为后台服务有依赖关系，所以需要先执行 start_01.bat 脚本，确保 sys 控制台启动完成后，才能执行 start_02.ba t脚本：
